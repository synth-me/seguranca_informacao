...
def comparar_hash(d_mensagem:dict,file_name:str):
    h = {}
    for i in d_mensagem:
        h_256 = hashlib.sha256(i.encode("utf-8")).hexdigest()
        h_md5 = hashlib.md5(i.encode("utf-8")).hexdigest()
        h[i] = []
        for y in d_mensagem[i]:
            if h_256 == y or h_md5 == y:
                if h_256 == y:
                    h[i].append('{}: verdadeiro para sha256'.format(d_mensagem[i].index(y)+1))
                if h_md5 == y:
                    h[i].append('{}: verdadeiro para md5'.format(d_mensagem[i].index(y)+1))
            elif h_256 != y and h_md5 != y: 
                h[i].append('{}: falso para os dois'.format(d_mensagem[i].index(y)+1))

    j = json.dumps(h,indent=4)

    o = open("./{}.txt".format(file_name),"w")
    o.write(j)
    o.close()
...
if __name__ == "__main__":
    ...
    comparar_hash(gabarito,"arquivo_1")
    comparar_hash(list_h,"arquivo_2")
